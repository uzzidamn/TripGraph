...
# ── run.py (output inspection) ────────────────────────────────────────
RUN_CHAT=                    # optional: comma-separated chat messages (uses built-in sample if blank)
RUN_REPLAN=true              # true | false — set false to skip the replanning section
```
